...
const SLIDE_INTERVAL = 7000; // 7 secondsav
...
export default function Home() {
  ...
  useEffect(() => {
    const heroImages =
      hero?.imageUrls || (hero?.imageUrl ? [hero.imageUrl] : []);
      ...
      <section className="relative h-[30vh] md:h-screen min-h-[220px] md:min-h-[600px] bg-[#065f46] overflow-hidden">
        ...
        <div className="relative z-10 h-full flex flex-col items-center justify-center text-center text-white px-4 max-w-4xl mx-auto">
          ...
          <p className="text-xs sm:text-sm md:text-lg lg:text-2xl text-white/90 mb-6 md:mb-12 drop-shadow-lg max-w-2xl mx-auto font-medium">
            {hero?.subtitle ||
              "Discover premium export products across multiple categories"}
          </p>
          <div className="flex flex-col sm:flex-row gap-2.5 md:gap-5 justify-center px-6 sm:px-0">
            <Link
              to="/products"
              className="px-4 py-2.5 sm:px-6 sm:py-3 md:px-10 md:py-4 bg-[#10b981] hover:bg-[#059669] text-white font-bold rounded-lg transition-all shadow-xl active:scale-95 group text-xs sm:text-sm md:text-base"
            >
              Explore Products
            </Link>
            <Link
              to="/categories"
              className="px-4 py-2.5 sm:px-6 sm:py-3 md:px-10 md:py-4 bg-transparent hover:bg-white/10 text-white font-bold rounded-lg border border-white/40 transition-all active:scale-95 text-xs sm:text-sm md:text-base"
            >
              Get a Quote
            </Link>
          </div>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 overflow-hidden">
        ...
        <div className="flex items-center justify-between mb-12">
          <div>
            ...
            <p className="text-gray-500 mt-1 sm:mt-2 text-[13px] md:text-base">
              Find products organized by category
            </p>
          </div>
          {categories.length > 4 && (
