...
export default function ProductManager() {
    ...
    const handleSave = async (e) => {
        ...
        if (selectedUnits.length === 0) { toast.error('Please select at least one unit'); return; }
        if (!editId && imageFiles.length === 0) { toast.error('Please upload at least one image'); return; }
        if (editId && existingImages.length === 0 && imageFiles.length === 0) {
    ...
    return (
        <div className="space-y-6">
                    ...
                    <div className="flex flex-col sm:flex-row items-center gap-4 w-full lg:w-auto">
                        <div className="flex items-center gap-2 w-full sm:w-auto">
                            ...
                            <select
                                ...
                                className="w-full sm:w-auto px-3 py-2 border border-gray-200 rounded-lg outline-none focus:ring-2 focus:ring-[#10b981] text-sm bg-white"
                            >
                                        ...
                                        <tr key={product.id} className="hover:bg-gray-50/50 transition-colors">
                                            ...
                                            <td className="px-6 py-4">
                                                <div className="flex items-center gap-4">
                                                    <div className="w-20 h-20 rounded-lg overflow-hidden flex-shrink-0 border border-black/5 bg-gray-50">
                                                        <ImageWithFallback
                                                    ...
                                                    <div className="min-w-0">
                                                        <p className="font-semibold text-gray-900 truncate" title={product.title}>
                                            ...
                                            <td className="px-6 py-4 text-gray-600 hidden md:table-cell">
                                                <span className="px-2.5 py-1 bg-gray-100 rounded-full text-xs font-medium truncate inline-block max-w-[150px]" title={getCategoryName(product.categoryId)}>
                                                    {getCategoryName(product.categoryId).length > 20 ? getCategoryName(product.categoryId).substring(0, 20) + '...' : getCategoryName(product.categoryId)}
                                                </span>
                                            </td>
                                            ...
                                            <td className="px-6 py-4 text-right">
                                                <div className="flex items-center justify-end gap-1">
                                                    <button onClick={() => openEdit(product)} className="p-2 text-gray-400 hover:text-green-700 hover:bg-green-50 rounded-lg transition-all">
                                                        <HiOutlinePencil className="w-4 h-4" />
                                                    </button>
                                                    <button onClick={() => handleDeleteClick(product.id, product.title)} title="Delete Product" className="p-2 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-all">
            ...
            <DeleteConfirmation
                ...
                message="Are you sure you want to delete this product? This will remove it from the catalog and all categories."
